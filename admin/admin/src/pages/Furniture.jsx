...
function Furniture() {
  ...
  const deleteFurniture = async (id) => {
    ...
    try {
      ...
      alert("Furniture item deleted successfully");
      loadData();
    } catch (err) {
      console.log(err);
      alert("Delete Failed");
  ...
  return (
    <AdminLayout>
      <div className="responsive-header" style={styles.responsiveHeader}>
        <h3 style={styles.title}>Furniture</h3>
        <button
          className="btn btn-primary"
          style={styles.addButton}
          onClick={() => navigate("/furniture/add")}
        >
          <span style={{ marginRight: "5px" }}>+</span> Add Furniture
        </button>
      </div>
                      ...
                      {furniture.length === 0 ? (
                        <tr>
                          <td colSpan="11" className="text-center py-4">
                            <p style={styles.emptyText}>No Furniture Found</p>
                          </td>
                        </tr>
                      ) : (
